...
class MultiModelHandler:
    ...
    def evaluate_ceval_val(self, model_choice, args, subjects=None, max_samples=None, run_id=""):
        """委托给CEvalTester执行评估"""
        tester = CEvalTester(
            self,
            ceval_val_path="./dataset/ceval-exam/val",
            ceval_results_dir="./out/ceval_val_results",
            run_id=run_id
        )
    ...
    def evaluate_mmlu_val(self, model_choice, args, subjects=None, max_samples=None, run_id=""):
        """委托给CEvalTester执行评估"""
        tester = CEvalTester(
            self,
            ceval_val_path="./dataset/MMLU_ceval/data/val",
            ceval_results_dir="./out/mmlu_val_results",
            run_id=run_id
        )
        return tester.evaluate(model_choice, args, subjects, max_samples)

    def evaluate_mmlu_test(self, model_choice, args, subjects=None, max_samples=None, run_id=""):
        """委托给CEvalTester执行评估"""
        tester = CEvalTester(
            self,
            ceval_val_path="./dataset/MMLU_ceval/data/test",
            ceval_results_dir="./out/mmlu_test_results",
            run_id=run_id
        )
        return tester.evaluate(model_choice, args, subjects, max_samples)
...
#6、8、
    ...
    [0, 1, 2, 4],[0, 1], [0, 2], [0, 4], [1, 2], [1, 4], [2, 4],
]
    ...
    for model_index, model_choice in enumerate(model_choice_list):
        ...
        ceval_val = "./dataset/ceval-exam/val"  # 替换为实际路径
        ceval_subjects_val = [f for f in os.listdir(ceval_val)
                     if f.endswith(".csv") and os.path.isfile(os.path.join(ceval_val, f))]
        ceval_subjects_val = ceval_subjects_val[:number_subjects]
        handler.evaluate_ceval_val(model_choice, args, ceval_subjects_val,max_samples=number_problems,run_id=run_id)
        #
        # ceval_test = "./dataset/ceval-exam/test"  # 替换为实际路径
        # ceval_subjects_test = [f for f in os.listdir(ceval_test)
        #              if f.endswith(".csv") and os.path.isfile(os.path.join(ceval_test, f))]
        # ceval_subjects_test = ceval_subjects_test[:number_subjects]
        # handler.evaluate_ceval_test(model_choice, args, ceval_subjects_test,max_samples=number_problems,run_id=run_id)

        MMLU_val = "./dataset/MMLU_ceval/data/val"  # 替换为实际路径
        MMLU_subjects_val = [f for f in os.listdir(MMLU_val)
                     if f.endswith(".csv") and os.path.isfile(os.path.join(MMLU_val, f))]
        MMLU_subjects_val = MMLU_subjects_val[:number_subjects]
        handler.evaluate_mmlu_val(model_choice, args, MMLU_subjects_val,max_samples=number_problems,run_id=run_id)

        # MMLU_test = "./dataset/MMLU_ceval/data/test"  # 替换为实际路径
        # MMLU_subjects_test = [f for f in os.listdir(MMLU_test)
        #              if f.endswith(".csv") and os.path.isfile(os.path.join(MMLU_test, f))]
        # MMLU_subjects_test = MMLU_subjects_test[:number_subjects]
        # handler.evaluate_mmlu_test(model_choice, args, MMLU_subjects_test,max_samples=number_problems,run_id=run_id)

        handler.evaluate_boolq(model_choice, args, max_samples=number_problems*number_subjects,run_id=run_id)


        handler.evaluate_simpleMath(model_choice, args, max_samples=number_problems,run_id=run_id)
